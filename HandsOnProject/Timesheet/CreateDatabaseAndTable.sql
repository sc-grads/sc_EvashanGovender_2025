...
-- Check if the TimesheetDB database exists
IF EXISTS (SELECT name FROM sys.databases WHERE name = 'TimesheetDB')
BEGIN
    PRINT 'TimesheetDB exists.';
END
ELSE
BEGIN
    -- Create the TimesheetDB database
    CREATE DATABASE TimesheetDB;
    PRINT 'TimesheetDB database created successfully.';
END
GO

-- Switch to the TimesheetDB database (regardless of whether it existed or was just created)
USE TimesheetDB;
GO
